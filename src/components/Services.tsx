...
const Services = () => {
  ...
  return (
    <section id="services" className="py-20 bg-gradient-to-b from-gray-50 via-teal-100 to-teal-300 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto flex flex-col px-4 sm:px-6 lg:px-8 gap-10 md:gap-10 ">
        {services.map((service, index) => (
          <div
            key={index}
            className={`grid md:grid-cols-2  md:gap-6 items-center ${
              service.reverse ? "md:grid-flow-col-dense" : ""
            }`}
          >
            {/* Content */}
            <div className={`md:space-y-6 space-y-2  ${service.reverse ? "md:col-start-2" : ""}`}>
              <h2 className="text-4xl lg:text-[2.7rem] font-bold text-gray-800 leading-tight">
            ...
            <div className={`relative ${service.reverse ? "lg:col-start-1" : ""}`}>
              <div className="relative">
                <AnimatedBlobImage
                  ...
                  alt={service.title}
                  blobPaths={blobPaths[index]}
                  duration={3 + index} // stagger speeds a bit
                />
